...
python evals/retrieval/small_pool.py                      # 小花园（候选池 1–50 张）：答案会不会被门槛挡掉
```
...
## 小花园（候选池 1–20 张，2026-09-15）

`small_pool.py`：同一份卡和标注，拼出很小的候选池——一张答案卡 + 与该查询无关的卡填到 N 张（`answer`），
N 张无关卡（`noise`），陷阱卡 + 无关卡（`trap`）。`answer` 只统计在 210 张完整花园里默认配置找得到的
（查询, 答案卡）对（单句集 + 多轮窗口集），换说法这类词法边界不算小花园的锅。每个格子 5 个随机种子。
结果文件 `results/small-pool-2026-09-15.json`。

**问题**：覆盖率闸的 IDF 按候选池算，花园里一张都没有的查询词拿最大 IDF。池子越小，缺失词和命中词的
IDF 差得越离谱（1 张卡时 1.386 对 0.288），正确答案的覆盖率被压到闸下。新用户的花园正是这么小。

**做法**：覆盖率的 IDF 至少按 `F` 张卡的候选池算（多出来的是不含任何查询词的虚拟卡，`coverage_pool_floor`）。
只动覆盖率闸：分数、顺序、强证据闸照真实候选数算；候选池 ≥ F 时逐项不变，所以上面 210 张卡的所有数字不受影响。

`search` = `rank` 默认参数；`recall` = `select_context` + `strong_evidence=0.5`（一个宿主为自动想起放宽强证据闸的配置）。
`cjk-words` 是词级分词器的**替身**（CJK 只出相邻二字、不出单字），近似 jieba 这类按词切的分词器，不是 jieba。

| 找到答案 F=1 → F=20 | N=1 | 2 | 3 | 5 | 10 | 20 |
|---|---|---|---|---|---|---|
| 默认分词器 · search | 0.806 → **0.917** | 0.944 → 0.972 | 0.967 → 0.994 | 0.975 → 1.000 | 0.986 → 1.000 | 1.000 → 1.000 |
| 默认分词器 · recall | 0.959 → 0.986 | 0.986 → 0.986 | 1.000 | 1.000 | 1.000 | 1.000 |
| cjk-words · search | 0.508 → **0.814** | 0.915 → 0.949 | 0.922 → 0.956 | 0.963 → 0.980 | 0.966 → 0.983 | 0.997 |
| cjk-words · recall | 0.800 → 0.843 | 0.940 | 0.957 | 0.969 | 0.991 | 0.994 |

| 无关卡被返回 F=1 → F=20 | N=1 | 2 | 3 | 5 | 10 | 20 |
|---|---|---|---|---|---|---|
| 默认分词器 · search | 0.006 → 0.009 | 0.087 → 0.093 | 0.104 | 0.209 | 0.270 | 0.417 |
| 默认分词器 · recall | 0.061 | 0.238 → 0.243 | 0.310 → 0.313 | 0.429 | 0.583 | 0.670 |
| cjk-words · search | 0 | 0 | 0 | 0.009 | 0.026 → 0.032 | 0.043 |
| cjk-words · recall | 0 | 0.026 | 0.043 → 0.046 | 0.107 | 0.194 | 0.310 |

陷阱卡被返回的比例在 F=1 和 F=20 下完全相同（两种分词器、两条路）。

怎么定的 20：扫了 F ∈ {1, 5, 10, 15, 20, 30, 50, 100}。找到答案的比例在 F=15 到顶，15–100 结果逐格相同；
无关卡被返回在 F ≥ 15 时多出同样的 0.3–0.6 个百分点。取拐点之上的整数 20：再大在这份语料上没有收益，
却会把效果延伸到 20 张以上、没有评测证据的中等花园。

读法：

- recall 路本来就松（`strong_evidence=0.5`），小池子里的答案大多靠强证据闸进来，下限帮得少；search 路和
  词级分词器受益最大——正是「新用户问一句，花园里明明有却说没有」的形状。
- 小池子里「无关卡被返回」随 N 上升是另一件事：池子里全是无关卡时，泛词撞上也会过闸（大花园同样存在，
  见上文多轮窗口）。下限几乎不改变它。
- 合成语料、一个替身分词器、5 个种子——**只说明方向**；宿主要用自己的分词器（`--tokenizer path.py:TOK`）重测。
