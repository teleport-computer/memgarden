...
#: 一次 Dream 最多渲染几张卡、卡片区总共多少字符。和宿主 io V2 当前的预算一致
#: （``_MEMORY_CARDS_LIMIT`` 60 / ``_DREAM_CARDS_MAX_CHARS`` 60000）。
DEFAULT_DREAM_CARDS_LIMIT = 60
DEFAULT_DREAM_CARDS_BUDGET_CHARS = 60_000
#: 单卡正文上限。取 io 写入端的正文上限（5000 字），所以按 io 规则写出的卡
#: 永远完整呈现；只有正文更长的宿主数据才会被截断并标出。再低就会把正常的
