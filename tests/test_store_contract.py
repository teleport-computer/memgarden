...
from memgarden.storage import MutationRejected, FULL_CAPABILITIES, StoragePort  # noqa: E402
...
def test_write_then_read_back(store):
    store.apply(T, [_add("我有一只狗")], owner="owner-1", idempotency_key="k1")
    cards = store.load(T, owner="owner-1").cards
    assert [c["summary"] for c in cards] == ["我有一只狗"]


def test_tenants_are_isolated(store):
    store.apply(T, [_add("A 的记忆")], owner="owner-1", idempotency_key="k1")
    store.apply("tenant_b", [_add("B 的记忆")], owner="owner-1", idempotency_key="k1")
    assert [c["summary"] for c in store.load(T, owner="owner-1").cards] == ["A 的记忆"]
    assert [c["summary"] for c in store.load("tenant_b", owner="owner-1").cards] == ["B 的记忆"]
...
def test_revision_advances_after_each_write(store):
    r0 = store.load(T, owner="owner-1").revision
    r1 = store.apply(T, [_add("一")], owner="owner-1", idempotency_key="k1").revision
    r2 = store.apply(T, [_add("二")], owner="owner-1", idempotency_key="k2").revision
    assert r0 != r1 != r2


def test_stale_expected_revision_is_rejected(store):
    store.apply(T, [_add("一")], owner="owner-1", idempotency_key="k1")
    with pytest.raises(RuntimeError):
        store.apply(T, [_add("二")], owner="owner-1", idempotency_key="k2", expected_revision="0")


def test_fresh_expected_revision_is_accepted(store):
    rev = store.load(T, owner="owner-1").revision
    store.apply(T, [_add("一")], owner="owner-1", idempotency_key="k1", expected_revision=rev)
    assert len(store.load(T, owner="owner-1").cards) == 1
...
def test_replaying_the_same_key_does_not_write_twice(store):
    first = store.apply(T, [_add("只该有一张")], owner="owner-1", idempotency_key="same")
    again = store.apply(T, [_add("只该有一张")], owner="owner-1", idempotency_key="same")
    assert len(store.load(T, owner="owner-1").cards) == 1, "同一个幂等键重放写了两次"
...
def test_different_keys_write_separately(store):
    store.apply(T, [_add("一")], owner="owner-1", idempotency_key="k1")
    store.apply(T, [_add("二")], owner="owner-1", idempotency_key="k2")
    assert len(store.load(T, owner="owner-1").cards) == 2
...
def test_supersede_archives_the_old_card_instead_of_deleting(store):
    res = store.apply(T, [_add("旧的说法")], owner="owner-1", idempotency_key="k1")
    ...
    visible = store.load(T, owner="owner-1").cards
    assert [c["summary"] for c in visible] == ["新的说法"], "旧卡还露在外面"

    everything = store.load(T, owner="owner-1", include_archived=True, include_superseded=True).cards
...
def test_a_failed_mutation_rolls_back_the_whole_batch(store):
    store.apply(T, [_add("先有一张")], owner="owner-1", idempotency_key="k1")
...
def test_unknown_op_is_rejected(store):
    with pytest.raises(ValueError):
        store.apply(T, [{"op": "打个响指"}], owner="owner-1", idempotency_key="k1")
...
def test_deleting_a_card_never_lets_the_next_one_overwrite_a_survivor(store):
    ...
    """
    t = "tenant-overwrite"
    store.apply(t, [{"op": "add", "card": {"summary": "first"}}], owner="owner-1", idempotency_key="k1")
    store.apply(t, [{"op": "add", "card": {"summary": "second"}}], owner="owner-1", idempotency_key="k2")
    survivors_before = {c["summary"] for c in store.load(t, owner="owner-1").cards}

    store.apply(t, [{"op": "delete", "target_id": "m_1"}], owner="owner-1", idempotency_key="k3")
    store.apply(t, [{"op": "add", "card": {"summary": "third"}}], owner="owner-1", idempotency_key="k4")

    after = {c["summary"] for c in store.load(t, owner="owner-1").cards}
...
def test_the_same_idempotency_key_with_different_content_is_a_conflict(store):
    ...
    t = "tenant-idem"
    store.apply(t, [{"op": "add", "card": {"summary": "A"}}], owner="owner-1", idempotency_key="same")

    with pytest.raises(IdempotencyConflict):
        store.apply(t, [{"op": "add", "card": {"summary": "B 完全不同"}}],
                    owner="owner-1", idempotency_key="same")
...
def test_a_genuine_replay_is_still_idempotent(store):
    ...
    first = store.apply(t, mutations, owner="owner-1", idempotency_key="same")
    again = store.apply(t, list(mutations), owner="owner-1", idempotency_key="same")

    assert again.results == first.results
    assert again.revision == first.revision
    assert len(store.load(t, owner="owner-1").cards) == 1, "重放写进了第二份"
