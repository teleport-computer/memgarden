...
解析对模型输出的容错范围是有限的：Capture/Dream/Migrate 从回复中取第一个完整 JSON 对象，允许前后有说明文字、Markdown 代码围栏或推理块，字符串里的 `{` `}` 不影响取块。只有 Capture 在直接解析失败后会尝试补转义字符串内的裸引号，且只修引号后面跟普通文字的形态（如 `她说"好的"然后走了`）。引号后面紧跟 `,` `}` `]` `:`（如 `她说"好的", 然后`）与字段结束无法区分，不修；引号后面紧跟另一个引号（多为漏逗号，如 `["a" "b"]`）有歧义，也不修（唯一例外是引语恰好在值末尾，如 `"他只说了"算了""` 后接 `,` `}` `]`）。引号后面是 `{` `[`，或是完整的数字/`true`/`false`/`null` 且紧跟 `,` `}` `]`（多为漏冒号，如 `"importance" 0.7,`），同样不修；`他报价"1000"块` 这类正文照修。值末尾多敲一个引号（`["好的"","她说"]`）会被读成 `好的"`，这是已知局限。这些情况都报 `json_decode_error`（Capture 与 Maintenance 会按现有重试预算重问），修复不会把它们猜成另一种意思落库。
