...
- **History Import**：`history_import` 明确声明为 `false`。指定不支持的
  policy 会被拒绝，**不会**拿日常聊天那把尺子假装完成导入 —— 那会产生
  「导入成功但几乎没记住」的假成功。分批、游标、断点续跑是下一阶段的事。
- **部分失败的表达**：一批 mutation 里前几条成功、后面失败时，调用方现在
  只看得到一个异常，看不到哪几条已经落库。两个官方 Store 都是原子的，
  所以目前不会出现这个状态；但外部适配器做不到原子时，接口还没有话说。
